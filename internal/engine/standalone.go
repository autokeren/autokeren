...
func RunStandaloneEventsWithRouterState(ctx context.Context, cfg config.Config, root, prompt string, events Events, resume string, routerState *provider.RouterState) (string, error) {
	...
	registry := tool.NewRegistry().Register(tool.Environment{}).Register(tool.ReadFile{Root: root}).Register(tool.WriteFile{Root: root, Guard: writeGuard}).Register(tool.PatchFile{Root: root, Guard: writeGuard}).Register(tool.ListFiles{Root: root}).Register(tool.SearchCode{Root: root}).Register(tool.GitStatus{Root: root}).Register(tool.GitDiff{Root: root}).Register(tool.GitLog{Root: root}).Register(tool.GitCommit{Root: root}).Register(tool.GitBranch{Root: root}).Register(tool.GitAutoCommit{Root: root}).Register(tool.NewTodoList(root)).Register(tool.NewKanban(root)).Register(tool.Proof{Root: root}).Register(tool.Remember{Root: root}).Register(tool.FetchURL{}).Register(tool.CFDeploy{Root: root}).Register(tool.CFBuild{Root: root}).Register(tool.CFVerify{Root: root, Browser: sharedBrowser}).Register(tool.Genome{Root: root}).Register(tool.CreateProject{Config: cfg}).Register(tool.DeployProject{Config: cfg, Root: root}).Register(tool.ListProjects{Config: cfg}).Register(tool.ScaffoldApp{Root: root, Guard: writeGuard}).Register(tool.PublishApp{Config: cfg, Root: root}).Register(tool.AppReleaseStatus{Config: cfg}).Register(tool.RepoMap{Root: root}).Register(tool.CFKV{Config: cfg}).Register(tool.CFD1{Config: cfg}).Register(tool.Browser{Manager: sharedBrowser}).Register(tool.Research{}).Register(tool.Review{Root: root}).Register(tool.SecurityScan{Root: root}).Register(tool.Rewind{Manager: checkpoints}).Register(tool.SpawnGhost{Manager: ghostManager}).Register(tool.AwaitAgents{Coordinator: coordinator}).Register(tool.Collaborate{Manager: ghostManager}).Register(tool.CheckGhost{Manager: ghostManager}).Register(tool.StopGhost{Manager: ghostManager}).Register(tool.Shell{Root: root})
