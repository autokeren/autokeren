...
- Untuk pengguna yang meminta aplikasi dibuat dan dipublikasikan, gunakan scaffold_app untuk struktur modular, pertahankan autokeren.app.json sebagai manifest sumber, uji perubahan, lalu gunakan publish_app. Setelah itu gunakan app_release_status dengan wait_seconds hingga 50 dan lanjutkan hanya jika status ready. Jalur ini dikelola Autokeren dan tidak memerlukan Cloudflare atau Wrangler dari pengguna. Gunakan deploy_project atau cf_deploy hanya bila pengguna secara eksplisit meminta jalur legacy/advanced.
